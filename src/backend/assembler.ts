...
export function assembleDetailed(
    source: string,
    options?: AnalyzerOptions
): AssembleDetailedResult {
    ...
    return {
        output: words
            .map((w) => formatNumber(w, options?.numberBase ?? "hex"))
            .join("\n"),
        words,
        detectedXlen,
        mode,
...
class Assembler {
    ...
    public assemble(source: string): { words: number[]; detectedXlen: XLen } {
        ...
        for (let i = 0; i < lines.length; i += 1) {
            ...
            const produced = this.assembleInstructionOrPseudo(
                mnemonic,
                operands,
                lineNumber
            )
    ...
    private assembleInstructionOrPseudo(
        mnemonic: string,
        operands: string[],
        line: number
    ): number[] {
        ...
        for (const inst of expanded) {
            ...
            if (!innerSpec) {
                throw new AnalyzerError(
                    `Internal error expanding pseudo '${mnemonic}' -> '${inst.mnemonic}'`,
                    line
                )
            }
            words.push(
                this.assembleSingle(innerSpec, inst.operands, line) >>> 0
            )
        }
        return words
    }
        ...
        switch (spec.operandPattern) {
            ...
            case "rd_rs1":
                return spec.format === "I"
                    ? assembleIType(spec, operands, line, this.isEmbedded)
                    : assembleRType(spec, operands, line, this.isEmbedded)
            case "rd_rs1_imm12":
...
function ensureOperandCount(
    ...
    if (operands.length !== expected) {
        throw new AnalyzerError(
            `Expected ${expected} operand(s) for ${description}, received ${operands.length}`,
            line
        )
    }
}
...
function parseFloatRegisterOperand(
    token: string,
    role: string,
    line: number
): number {
    try {
        ...
        throw new AnalyzerError(
            `Invalid ${role} float register '${token}'`,
            line
        )
    }
}

function assembleRType(
    spec: InstructionSpec,
    operands: string[],
    line: number,
    isEmbedded: boolean
...
function assembleFloatRType(
    ...
    switch (spec.operandPattern) {
        ...
        default:
            throw new AnalyzerError(
                `Unsupported float operand pattern for '${spec.name}'`,
                line
            )
    }
...
function assembleFloatR4Type(
    ...
    if (operands.length !== 4) {
        throw new AnalyzerError(
            `Expected 4 operands, got ${operands.length}`,
            line
        )
    }
...
function assembleUType(
    ...
    return ((imm & immMask) << 12) | ((rd & 0x1f) << 7) | (spec.opcode & 0x7f)
}

function assembleJump(
    spec: InstructionSpec,
    operands: string[],
    line: number,
    isEmbedded: boolean
...
function assembleZeroOperand(
    ...
    return (
        (imm << 20) | (((spec.funct3 ?? 0) & 0x7) << 12) | (spec.opcode & 0x7f)
    )
}

function assembleFence(
    spec: InstructionSpec,
    operands: string[],
    line: number
): number {
    if (operands.length !== 0 && operands.length !== 2) {
        throw new AnalyzerError(
            `Expected 0 or 2 operand(s) for ${spec.name}, received ${operands.length}`,
            line
        )
    }
    let predMask = 0xf
...
function assembleCsrRegister(
    ...
    ensureOperandCount(operands, 3, line, spec.name)
    const rd = parseRegisterOperand(operands[0], "rd", line, isEmbedded)
    const csr = parseImmediate(operands[1], {
        bits: 12,
        signed: false,
        line,
        label: "csr",
...
function assembleCsrImmediate(
    ...
    const zimm = parseImmediate(operands[2], {
        bits: 5,
        signed: false,
        line,
        label: "immediate",
...
function parseMemoryOperand(
    ...
    const offset = parseImmediate(offsetToken, {
        bits: 12,
        signed: true,
        line,
        label: "offset",
...
function assembleVType(
    ...
    switch (spec.operandPattern) {
        ...
        default:
            throw new AnalyzerError(
                `Unsupported vector operand pattern '${spec.operandPattern}'`,
                line
            )
    }
    ...
    switch (m) {
        ...
        case "li": {
            ...
            if (fitsSigned(imm, 12)) {
                return [
                    { mnemonic: "addi", operands: [dest, "x0", String(imm)] },
                ]
            }
            ...
            if (!fitsSigned(lower, 12)) {
                throw new AnalyzerError(
                    `Cannot encode immediate ${imm} in 12-bit addi after LUI`,
                    line
                )
            }
            ...
            if (upper !== 0) {
                ...
                if (lower !== 0) {
                    seq.push({
                        mnemonic: "addi",
                        operands: [dest, dest, String(lower)],
                    })
                }
            } else {
