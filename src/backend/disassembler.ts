...
export function disassembleDetailed(
    source: string,
    options?: AnalyzerOptions
): DisassembleDetailedResult {
...
class Disassembler {
    ...
    public disassemble(tokens: MachineWordToken[]): {
        lines: string[]
        detectedXlen: XLen
    } {
        const lines: string[] = []
        ...
        const detected =
            this.mode === "auto" ? this.detectedXlen : (this.mode as XLen)
    ...
    private disassembleWord(word: number, line: number): string {
        ...
        if (!candidates || candidates.length === 0) {
            throw new AnalyzerError(
                `Unknown opcode ${formatNumber(opcode)}`,
                line
            )
        }
        const filtered = candidates
            .filter((candidate) => this.isInstructionAllowed(candidate))
            .sort((a, b) => (b.minXlen ?? 32) - (a.minXlen ?? 32))
        if (filtered.length === 0) {
            const required = Math.min(
                ...candidates.map((spec) => spec.minXlen ?? 32)
            )
        ...
        switch (spec.operandPattern) {
            ...
            case "rd_rs1":
                return spec.format === "I"
                    ? decodeIType.call(this, spec, word)
                    : decodeRType(spec, word)
            case "rd_rs1_imm12":
            ...
            default:
                throw new AnalyzerError(
                    `Unhandled operand pattern for '${spec.name}'`,
                    line
                )
        }
    }
    ...
    private disassembleCompressedWord(word: number, line: number): string {
        ...
        switch (opcode) {
            ...
            default:
                throw new AnalyzerError(
                    "Invalid compressed instruction encoding",
                    line
                )
        }
        ...
        if (!candidates || candidates.length === 0) {
            throw new AnalyzerError(
                `Unknown compressed opcode ${formatNumber(fullOpcode)}`,
                line
            )
        }
        const filtered = candidates
        ...
        if (filtered.length === 0) {
            throw new AnalyzerError(
                "Unsupported compressed instruction encoding",
                line
            )
        }
        const spec = filtered.find((candidate) =>
            matchesCompressedSpec(candidate, word)
        )
        if (!spec) {
            throw new AnalyzerError(
                "Unsupported compressed instruction encoding",
        ...
        switch (spec.operandPattern) {
            ...
            default:
                throw new AnalyzerError(
                    `Unhandled compressed operand pattern for '${spec.name}'`,
                    line
                )
        }
    }
...
function matchesSpec(spec: InstructionSpec, word: number): boolean {
    ...
    if (spec.fixedImmediate !== undefined) {
        ...
        if (
            (imm & ((1 << (spec.immBits ?? 12)) - 1)) !==
            (spec.fixedImmediate & ((1 << (spec.immBits ?? 12)) - 1))
        ) {
            return false
        }
    }
...
function decodeRType(spec: InstructionSpec, word: number): string {
    ...
    if (spec.operandPattern === "rd_rs1") {
        ...
        return `${spec.name} ${formatRegister(rd)}, ${formatRegister(
            rs1
        )}, ${formatRegister(rs2)}`
    }
}
...
function decodeFloatRType(spec: InstructionSpec, word: number): string {
    ...
    switch (spec.operandPattern) {
        case "fd_fs1_fs2":
            return `${spec.name} ${formatFloatRegister(
                fd
            )}, ${formatFloatRegister(fs1)}, ${formatFloatRegister(fs2)}`
        case "fd_fs1":
            return `${spec.name} ${formatFloatRegister(
                fd
            )}, ${formatFloatRegister(fs1)}`
        case "fd_rs1":
            return `${spec.name} ${formatFloatRegister(fd)}, ${formatRegister(
                fs1
            )}`
        case "rd_fs1":
            return `${spec.name} ${formatRegister(fd)}, ${formatFloatRegister(
                fs1
            )}`
        case "rd_fs1_fs2":
            return `${spec.name} ${formatRegister(fd)}, ${formatFloatRegister(
                fs1
            )}, ${formatFloatRegister(fs2)}`
        default:
...
function decodeFloatR4Type(spec: InstructionSpec, word: number): string {
    ...
    return `${spec.name} ${formatFloatRegister(fd)}, ${formatFloatRegister(
        fs1
    )}, ${formatFloatRegister(fs2)}, ${formatFloatRegister(fs3)}`
}

function decodeFloatIType(
    this: Disassembler,
    ...
    return `${spec.name} ${formatFloatRegister(fd)}, ${formatNumber(
        imm,
        this.numberBase
    )}(${formatRegister(rs1)})`
}

function decodeFloatSType(
    this: Disassembler,
    spec: InstructionSpec,
    word: number
): string {
    ...
    return `${spec.name} ${formatFloatRegister(fs2)}, ${formatNumber(
        imm,
        this.numberBase
    )}(${formatRegister(rs1)})`
}

function decodeIType(
    this: Disassembler,
    spec: InstructionSpec,
    word: number
): string {
    ...
    if (spec.fixedImmediate !== undefined) {
        ...
        return `${spec.name} ${formatRegister(rd)}, ${formatRegister(
            rs1
        )}, ${formatNumber(imm, this.numberBase)}`
    }
}
...
function decodeLoad(
    ...
    return `${spec.name} ${rdStr}, ${formatNumber(
        offset,
        this.numberBase
    )}(${formatRegister(rs1)})`
}
...
function decodeStore(
    ...
    return `${spec.name} ${rs2Str}, ${formatNumber(
        offset,
        this.numberBase
    )}(${formatRegister(rs1)})`
}
...
function decodeBranch(
    ...
    return `${spec.name} ${formatRegister(rs1)}, ${formatRegister(
        rs2
    )}, ${formatNumber(imm, this.numberBase)}`
}

function decodeUType(
    this: Disassembler,
    spec: InstructionSpec,
    word: number
): string {
    ...
    return `${spec.name} ${formatRegister(rd)}, ${formatNumber(
        imm,
        this.numberBase
    )}`
}
...
function decodeCsrImmediate(
    ...
    return `${spec.name} ${formatRegister(rd)}, ${formatNumber(
        csr,
        this.numberBase
    )}, ${formatNumber(zimm, this.numberBase)}`
}
...
function decodeCSSType(
    ...
    return `${spec.name} ${formatRegister(rs2)}, ${formatNumber(
        imm,
        this.numberBase
    )}`
}

function decodeCIWType(
    this: Disassembler,
    spec: InstructionSpec,
    word: number
): string {
...
function decodeCBType(
    ...
    if (spec.operandPattern === "rd_rs1_nzimm5") {
        ...
        return `${spec.name} ${formatRegister(rs1)}, ${formatNumber(
            imm,
            this.numberBase
        )}`
    }
...
function decodeVType(
    ...
    switch (spec.operandPattern) {
        case "vd_vs1_vs2":
        case "vd_vs1_vs2_vm":
            operands = `${formatVectorRegister(vd)}, ${formatVectorRegister(
                vs1
            )}, ${formatVectorRegister(vs2)}`
            break
        case "vd_vs1_rs2":
        case "vd_vs1_rs2_vm":
            operands = `${formatVectorRegister(vd)}, ${formatVectorRegister(
        ...
        case "vd_vs1_imm_vm":
            ...
            operands = `${formatVectorRegister(vd)}, ${formatVectorRegister(
                vs1 >> 5 ? vs1 : 0
            )}, ${formatNumber(imm, this.numberBase)}` // For imm, vs1 field contains immediate
            break
        case "vd_rs1":
        case "vd_rs1_vm":
...
function decodeVLoad(
    ...
    const rs1 = (word >> 15) & 0x1f
    const imm = signExtend((word >> 20) & 0xfff, 12)
    return `${spec.name} ${formatVectorRegister(vd)}, ${formatNumber(
        imm,
        this.numberBase
    )}(${formatRegister(rs1)})`
}

function decodeVStore(
    this: Disassembler,
    spec: InstructionSpec,
    word: number
): string {
    ...
    return `${spec.name} ${formatVectorRegister(vs2)}, ${formatNumber(
        imm,
        this.numberBase
    )}(${formatRegister(rs1)})`
}
