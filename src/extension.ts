...
type OutboundMessage =
    ...
    | { type: "setNumberBase"; value: "hex" | "dec" }
...
interface UpdateNumberBaseMessage {
    type: "updateNumberBase"
    value: "hex" | "dec"
}
...
type InboundMessage =
    ...
    | UpdateXlenMessage
    | UpdateEmbeddedMessage
    | UpdateFloatMessage
    | UpdateNumberBaseMessage
...
class RiscvAnalyzerViewProvider implements vscode.WebviewViewProvider {
    ...
    private floatMode: FloatMode = "auto"
    private numberBase: "hex" | "dec" = "hex"
    ...
    public async reveal(preserveFocus = false): Promise<void> {
        await vscode.commands.executeCommand(
            "workbench.view.extension.riscvAsmAnalyzer"
        )
            ...
            case "updateNumberBase":
                ...
                try {
                    await vscode.workspace
                        .getConfiguration("riscvAsmAnalyzer")
                        .update(
                            "numberBase",
                            message.value,
    ...
    private resolveFloatOptions(
        floatMode: FloatMode,
        input: string
    ): { floatEnabled: boolean; doubleEnabled: boolean; quadEnabled: boolean } {
        switch (floatMode) {
            case "disabled":
                return {
                    floatEnabled: false,
            ...
            case "auto":
                ...
                return {
                    floatEnabled: hasSingle || hasDouble || hasQuad,
                    doubleEnabled: hasDouble || hasQuad,
                    quadEnabled: hasQuad,
                }
            default:
                return {
    ...
    private async handleRun(
        mode: AnalyzerMode,
        input: string,
        xlenMode: XLenSelection,
        isEmbedded: boolean,
        floatMode: FloatMode,
        numberBase: "hex" | "dec" = "hex"
    ): Promise<void> {
        const { floatEnabled, doubleEnabled, quadEnabled } =
            this.resolveFloatOptions(floatMode, input)
        if (!input.trim()) {
            this.enqueueMessage({
                type: "error",
                value: l10n.t("Input is empty."),
            })
            return
        }
        ...
        try {
            ...
            const modeLabel =
                mode === "assemble" ? l10n.t("assembly") : l10n.t("disassembly")
            vscode.window.showErrorMessage(
                l10n.t("RISC-V {0} failed: {1}", modeLabel, message)
            )
        } finally {
    ...
    private async handleCopy(value: string): Promise<void> {
        ...
        vscode.window.setStatusBarMessage(
            l10n.t("RISC-V analyzer output copied to clipboard."),
            1500
        )
    }
    ...
    private pushActiveSelection(): void {
        ...
        this.enqueueMessage({
            type: "info",
            value: l10n.t("No active editor selection to load."),
        })
    }
    ...
    private buildHtml(webview: vscode.Webview): string {
        ...
        const uiStrings = {
            ...
            runButtonHint: l10n.t(
                "Click to assemble. Hold Alt and click to disassemble."
            ),
            copyButtonLabel: l10n.t("Copy Output"),
            clearButtonLabel: l10n.t("Clear"),
            disassemblerHint: l10n.t(
                "Alt+Click the main button to run the disassembler."
            ),
            ...
            xlenHint: l10n.t(
                "Choose the register width for encoding and decoding."
            ),
            embeddedLabel: l10n.t("Embedded"),
            embeddedOptionStandard: l10n.t("Standard"),
            embeddedOptionEmbedded: l10n.t("Embedded"),
            embeddedHint: l10n.t(
                "Choose whether to use embedded architecture (16 registers) or standard architecture (32 registers)."
            ),
            ...
            floatHint: l10n.t(
                "Enable RISC-V Floating-Point Extensions (RVF/RVD/RVQ) instructions."
            ),
            numberBaseLabel: l10n.t("Number base"),
            numberBaseHex: l10n.t("Hex (0x1234)"),
            numberBaseDec: l10n.t("Decimal (12345)"),
        }
		...
		<body>
			<section>
				<header>
					...
					<a href="#" id="loadSelectionLink" class="hint">${escapeHtml(
                        uiStrings.loadSelectionLabel
                    )}</a>
				</header>
				<textarea id="inputArea" placeholder="${escapeAttribute(
                    uiStrings.inputPlaceholder
                )}"></textarea>
			</section>
			<section>
				<h2>${escapeHtml(uiStrings.outputHeading)}</h2>
				<textarea id="outputArea" placeholder="${escapeAttribute(
                    uiStrings.outputPlaceholder
                )}" readonly></textarea>
			</section>
			...
			<div class="controls-row">
				<label for="numberBaseSelect">${escapeHtml(uiStrings.numberBaseLabel)}</label>
				<select id="numberBaseSelect">
					<option value="hex">${escapeHtml(uiStrings.numberBaseHex)}</option>
					<option value="dec">${escapeHtml(uiStrings.numberBaseDec)}</option>
				</select>
			</div>
			<div class="button-row">
				<button id="runButton" title="${escapeAttribute(
                    uiStrings.runButtonHint
                )}">${escapeHtml(uiStrings.runButtonLabel)}</button>
...
export function activate(context: vscode.ExtensionContext): void {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider(
            RiscvAnalyzerViewProvider.viewType,
            provider
        )
    )
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand(
            "riscv-asm-analyzer.openSidebar",
            async () => {
    ...
    context.subscriptions.push(
        vscode.commands.registerTextEditorCommand(
            "riscv-asm-analyzer.loadSelection",
            async (editor) => {
                const selectionText = getActiveSelectionText(editor)
                if (!selectionText) {
                    vscode.window.showWarningMessage(
                        l10n.t(
                            "Select text in the active editor to send it to the RISC-V analyzer."
                        )
                    )
                    return
                }
...
async function runAnalyzer(
    mode: AnalyzerMode,
    input: string,
    selection: XLenSelection,
    isEmbedded: boolean = false,
    floatEnabled: boolean = false,
    doubleEnabled: boolean = false,
    quadEnabled: boolean = false,
    numberBase: "hex" | "dec" = "hex"
    ...
    if (mode === "assemble") {
        ...
        return {
            ...
            detectedXlen: result.detectedXlen,
            mode: result.mode,
        }
    }
...
function getActiveSelectionText(
    ...
    if (!selection || selection.isEmpty) {
        const line = editor.document.lineAt(
            selection?.active ?? editor.selection.active
        )
